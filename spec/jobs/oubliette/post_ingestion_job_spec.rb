...
RSpec.describe Oubliette::PostIngestionJob do
  ...
  let(:request) { {resource_id: preserved_file.id, content_path: '/tmp/moo'} }
  ...
  describe "#do_characterisation" do
    it "starts a characterisation job" do
      ...
      }.to start_channel(Oubliette::CharacterisationJob, request: hash_including(resource_id: preserved_file.id, content_path: '/tmp/moo'))
    end
  end
